...
**Status (2026-07-28):** All four steps are implemented. Step 1 lives in `repeatlab_audioplayers_service_handler.dart` (loop wraps use a dedicated `_performLoopWrapSeek` fast path plus a predictive one-shot `_loopWrapTimer` armed on every position update, rearmed on speed change/resume/loop enable, self-healed by the 200 ms background poll). Step 2 is done in the `../ap` fork's `WrappedMediaPlayer.swift` (zero-tolerance seek), wired into the app via a `audioplayers_darwin` path override in `pubspec.yaml`. Step 4's unit tests are in the `predictive loop wrap` and `native loop region` groups of the handler test. Step 3 (native ExoPlayer loop region, RL-73) shipped as the **`PlayerMessage` variant** (RL-54 decision): the fork's `ExoPlayerWrapper` schedules a media3 `PlayerMessage` at the loop-end position (expressed in ExoPlayer's playout position space, inverted from the Signalsmith anchors) whose handler seeks back to the loop start through the shared `performSeek` path — so the click grid re-anchors via the existing pending-anchor protocol — and emits a dedicated `audio.onLoopWrap` event (deliberately not `onSeekComplete`, which a pending user seek awaits). The app enables it via `AudioPlayer.setLoopRegion(...)` in `enableLoopMode`; the Dart predictive wrap stands down while native is active (a 200 ms watchdog with a 250 ms margin catches seeks past the loop end, which native messages don't fire on) and remains the full fallback on iOS/video. `ClippingMediaSource` + `REPEAT_MODE_ONE` was rejected: a gapless repeat never flushes the sink, so the click-track frame counter and Signalsmith position anchors would silently drift, and clip-relative positions would break every consumer.

Manual QA checklist for step 4: see the "Loop Re-Entry Without Delay (seamless-loop QA checklist)" section in `MANUAL_TEST.md` — short loop (<2 s) at 0.5×/1×/2× foreground and background on both platforms, plus the 0.5× loop-near-song-end edge, seek-past-end watchdog, paused loop editing, and pitch/metronome stability across wraps.
