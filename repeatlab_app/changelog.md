...
### Added
- **One real test clip per supported import format, and an E2E flow that imports each of them.** `assets/test/` now holds a ~1 s clip for every extension the pickers offer — audio mp3/m4a/aac/wav/flac/ogg/wma/opus/aiff, video mp4/mov/m4v/mkv/webm/avi (~120 KB in total; declared in `pubspec.yaml`, so they ship with the app — kept tiny for that reason; regeneration commands are in `assets/test/README.md`). New `add_song_formats_flow_test.dart` imports every clip through the real pipeline (faked picker → copy → FFmpeg conversion where SoLoud can't decode natively → SoLoud/media_kit probe → persist), so a decoder that rejects a format shows up as its own failing test. `TestMedia` gained `bundledAudioFormats`/`bundledVideoFormats` and `writeBundledAudioToTemp`/`writeBundledVideoToTemp(ext:)`; the add-video flow no longer skips itself and now asserts on the tile's `ic_video` `AppIcon` (it looked for `Icons.videocam`, which the tile hasn't used for a while). New `homeTileMediaIcon()` helper in `e2e_app.dart`.
